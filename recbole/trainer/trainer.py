...
class Trainer(AbstractTrainer):
    ...
    def _full_sort_batch_eval_customized_samples(self, batched_data):
        interaction, history_index, positive_u, positive_i = batched_data
        scores = self.model.customized_sort_predict(interaction.to(self.device))

        scores = scores.view(-1, 101)
        positive_i = np.zeros_like(positive_i)
        return interaction, scores, positive_u, positive_i
    ...
    @torch.no_grad()
    def evaluate_no_log(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        r"""Evaluate the model based on the eval data.
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            interaction, row_idx, positive_u, positive_i = batched_data
            if self.config['model'] != "MBHT":
                origin_scores = self.model.predict(interaction.to(self.device))
            else:
                origin_scores = self.model.customized_sort_predict(interaction.to(self.device))
            score_results.append(origin_scores.view(-1, 101))
            # attn_scores.append(attn_score[0].squeeze())
        scores = torch.cat(score_results, dim=0)
        return scores
