...
def objective_function_mhrec(config_dict=None, config_file_list=None, saved=True):
    r""" The default objective_function used in HyperTuning
...
def load_data_and_model(model_file, dataset_file=None, dataloader_file=None, gpu_id=0):
    ...
    if not config['scales']:
        if config['dataset'] == "ijcai_beh":
            config['scales'] = [5, 8, 40]
        elif config['dataset'] == "tmall_beh":
            config['scales'] = [10, 4, 20]
        elif config['dataset'] == "retail_beh":
            config['scales'] = [10, 4, 20]
    dataset = None
    if dataset_file:
